...
public class AsyncTaskActivity extends AppCompatActivity {
    ...
    //기존에는 어싱크태스크를 제공하여 백그라운드에서 원격의 이미지 파일을 다운로드 하는 등의
    //소비적인 작업을 메인 스레드에 영향을 주지 않으면서 처리하였다
    //그렇게 사용되던 AsyncTask가 메모리 누수등의 문제로 인해
    //2019년 11월 8일 Deplecate 되었다
    //하여 AsyncTask의 대체수단인 rxjava에 대해 알아보자
    //rxjava는 안드로이드 새로 생성시 기본적으로 제공되는 라이브러리가 아니기 때문에
    //build.gradle에서
    ...
    private void asyncTask() {
        ...
        //onpreExcute(task 시작 전 실행될 코드 여기서 작성)
        ...
        backgroundtask = Observable.fromCallable(new Callable<Object>() {
         ...
         .subscribe(new Consumer<Object>() {
             ...
             public void accept(Object o) throws Exception {
                 ...
                 //onPostExcute(task 끝난 후 실행될 코드 여기에 작성)
                 progress.dismiss();
             }
         });
    ...
    private String connectServer() {
        ...
        String addr = "http://192.168.108.1:8081/conn";
        ...
        try {
            ...
            //웹서버 연결 최대 시간을 5초로 설정
            connection.setConnectTimeout(5000);
            
            //웹서버로부터 데이터를 읽어오는 최대 시간을 5초로 설정
            ...
            //AndroidManifest에
            //useCleartextTraffic = "true" 추가하여 주어야 한다.
            //또한 android.permisson.INTERNET 권한을 추가하어
            //네트워크 접속 권한을 주어야 한다.
            connection.connect();
            ...
            resultMsg = "서버 연결 중 알 수 없는 에러가 발생하였습니다.";
            ...
            try {
                ...
                LogService.error(activity, ioEx.getMessage(), ioEx);
            }
        }

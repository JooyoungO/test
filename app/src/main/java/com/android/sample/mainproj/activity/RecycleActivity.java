...
public class RecycleActivity extends AppCompatActivity {
    ...
    기존의 ListView는 커스터마이징 하기에 힘들었고, 구조적인 문제로 성능상의 문제가 있었다
     RecyclerView는 ListView의 문제를 해결하기 위해 개발자에게 더 다양한 형태로 커스터마이징 할 수 있도록 개발
     RecyclerView와 ListView의 가장 큰 차이점은 Layoutmanager와 ViewHolder 패턴의 의무적인 사용
     Item에 대한 뷰의 변형이나 애니메이션을 할 수 있는 개념이 추가
     */

    private RecyclerView rv_item;

    /*
    TODO RecycleView.Adapter Example 1
    ...
    */
    ...
    private void init() {
        ...
        et_rv_item_name = findViewById(R.id.et_custom_item_name);

        et_rv_item_age = findViewById(R.id.et_custom_item_age);

        btn_rv_item_add = findViewById(R.id.btn_custom_item_add);

        rv_item = findViewById(R.id.rv_item);

        memberList = new ArrayList<>();

        recycleAdapter = new RecycleAdapter(this, memberList);
        ...
        recycleAdapter = new RecyclerView.Adapter() {
            ...
            public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
                ...
                tv_item_name.setText(memberList.get(position).getName());

                tv_item_age.setText(memberList.get(position).getAge());
    ...
    private View.OnClickListener listener_item_add = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            ...
            recycleAdapter.addItem(name, age);

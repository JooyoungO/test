...
public class BootReceiver extends BroadcastReceiver {
    ...
    @Override
    public void onReceive(Context context, Intent intent) {  //intent : 휴대폰이 켜졌다라는 이벤트를 가져온다

        //BroadCastReceiverActivity를 띄우도록 설정
        Intent bootIntent = new Intent(context, BroadCastReceiverActivity.class);

        //Activity 가 아닌 다른 곳에서 화면을 띄우려고 할 때 다음 플래그를 설정
